...
if [ ! "$CHECK" = "*" ]; then
    ...
    echo "zendeskTicketNumber: $TICKET_NUMBER" >>"$CONV/$KV.txt"
else
...
if [ ! "$CHECK" = "*" ]; then
    ...
    echo "zendeskLink: $TICKET_LINK" >>"$CONV/$KV.txt"
else
...
if [ "${TITLE: -10}" = " \`\`{=html}" ]; then
    TITLE_EDIT=${TITLE%??????????}
    echo "title: $TITLE_EDIT" >>"$CONV/$KV.txt"
else
    echo "title: $TITLE" >>"$CONV/$KV.txt"
fi
...
if [ "$HAS_AST" = '*' ]; then
    ...
    for ((i = 1; i < 11; i++)); do
        ...
        sed 's/^.\{4\}//g' "$CONV/no-ast-keys.txt" >>"$CONV/$KV.txt"
        rm "$CONV/$i.txt"
    done
    ...
    for ((i = 1; i < 11; i++)); do
        ...
        sed 's/^.\{4\}//g' "$CONV/ast-keys.txt" >>"$CONV/$KV.txt"
        rm "$CONV/$i.txt"
    done
